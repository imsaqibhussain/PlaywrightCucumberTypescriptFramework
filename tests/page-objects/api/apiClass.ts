...
export class APIClass {
    ...
    async removeBookagainstUser() {
        ...
        try {
            ...
            if (response.status === 204) { // Replace 204 with the expected status code
    ...
    async generateTokenWithWrongCredentials() {
        ...
        const apiUrl = details.generateToken.apiUrl
        // Define the username and password parameters
        const username = details.invalidUser.username
        const password = details.invalidUser.password
        // Create an object with the parameters
        const requestBody = {
            userName: username,
            password: password,
